...
/*   Updated: 2025/09/10 17:56:09 by carlopez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_aux_eat(t_philos *philo)
{
...
void	ft_eat(t_philos *philo)
{
	...
	if (!((philo->last_meal + philo->main->time_to_eat)
			* 0.5 > ft_get_time_ms()))
		usleep(100);
	if (philo->id % 2 == 0)
	{
...
void	*ft_simulation(void *data)
{
	...
	if (philo->id % 2 != 0)
		usleep(philo->main->time_to_eat * 1000);
	if (philo->id % 2 != 0 && philo->id == philo->main->num_philos)
		usleep(philo->main->time_to_eat * 1000);
	while (!ft_is_simulation_dead(philo->main)
		&& philo->main->num_meals != philo->num_meal)
	{
		ft_eat(philo);
		if (ft_is_simulation_dead(philo->main)
			|| philo->main->num_meals == philo->num_meal)
			break ;
